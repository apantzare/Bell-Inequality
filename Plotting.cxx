...
void Plotting::readFiles(){
  ...
  std::cout<< SFiter->first << std::endl;
...
void Plotting::makePlots(){
  ...
  for(; fIter != lIter; ++fIter){
    ...
    if(option.find("HWWAnalysis") != option.npos){
      ...
      //TEST AXEL
      backg = (TH1F*)stop->Clone();
      backg->Add(V);
      backg->Add(diboson);
      backg->Add(ttbar);            
      backg->SetFillColor(kAzure+8);
      backg->SetLineWidth(0);
    ...
    if(option.find("HWWAnalysis") != option.npos){
      ...
      histstack = (TH1F*)stop->Clone();
      histstack->Add(diboson);
      histstack->Add(V);
      //histstack = (TH1F*)backg->Clone();
      histstack->Add(stop);
      histstack->Add(Higgs);
    }
   ...
   if(option.find("SUSYAnalysis") != option.npos){
    ...
    if(! (option.find("HyyAnalysis") != option.npos) ){
      ...
      //h_ratio->Multiply((TH1F*)Higgs->Clone());
      //for(int bin=0;bin<=h_ratio->GetNcells();bin++){
      //  h_ratio->SetBinContent(bin, sqrt(h_ratio->GetBinContent(bin)));
      //}
      h_ratio->GetYaxis()->SetTitle("S/(S+B)");
