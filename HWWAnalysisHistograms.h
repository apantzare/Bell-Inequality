...
void HWWAnalysis::define_histograms()
{
  ...
  hist_term1        = new TH1F("hist_term1",   "term1 distribution; term1 (ll);Events / bin", 20, 0, 3.2);
...
void HWWAnalysis::FillHistogramsGlobal( double m, float w , TString s)
{
  ...
  if (s.Contains("hist_term1")) hist_term1->Fill(m,w);
